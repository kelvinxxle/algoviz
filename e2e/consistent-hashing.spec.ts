...
const WORKBENCH = '[data-testid="consistent-hashing-workbench"]';

test.describe("Consistent Hashing workbench", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/topics/consistent-hashing");
  ...
  test("renders the curated walkthrough on the first frame", async ({
    ...
    await expect(page.getByTestId("consistent-hashing-ring")).toBeVisible();
    await expect(page.getByTestId("ring-circle")).toBeVisible();
    await expect(page.getByTestId("step-position")).toContainText("STEP 1 /");
  });

  test("stepping forward places a virtual node on the ring", async ({
    page,
  }) => {
    await page.getByRole("button", { name: "Step forward" }).click();
    await expect(page.locator('[data-vnode="A#0"]')).toBeVisible();
  });
  ...
  test("custom input reruns the same engine on the sandbox ring", async ({
    page,
  }) => {
    const input = [
      "ring: 100",
