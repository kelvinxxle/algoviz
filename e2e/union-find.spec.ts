...
const WORKBENCH = '[data-testid="union-find-workbench"]';

test.describe("Union-Find workbench", () => {
  ...
  test("custom input reruns the same engine on the sandbox operations", async ({
    page,
  }) => {
    const input = ["union X Y", "find X"].join("\n");
  ...
  test("invalid sandbox input surfaces a parser error", async ({ page }) => {
    await page.getByLabel("Custom input").fill("connect A B");
    await page.getByRole("button", { name: "Run visualization" }).click();
