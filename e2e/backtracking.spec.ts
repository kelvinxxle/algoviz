...
const WORKBENCH = '[data-testid="backtracking-workbench"]';

test.describe("Backtracking workbench", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/topics/backtracking");
  ...
  test("renders the curated walkthrough on the first frame", async ({
    ...
    await expect(page.getByTestId("backtracking-tree")).toBeVisible();
    await expect(page.getByTestId("backtracking-board")).toBeVisible();
    await expect(page.getByTestId("step-position")).toContainText("STEP 1 /");
    await expect(page.locator('[data-node="root"]')).toBeVisible();
  });
  ...
  test("custom input reruns the same engine on a new board size", async ({
    page,
  }) => {
    await page.getByLabel("Custom input").fill("n: 5");
  ...
  test("invalid sandbox input surfaces a parser error", async ({ page }) => {
    await page.getByLabel("Custom input").fill("9");
    await page.getByRole("button", { name: "Run visualization" }).click();
